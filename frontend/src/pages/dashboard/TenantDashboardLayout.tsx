...
// Near-exact copy of DashboardLayout.tsx (the platform super-admin's
// version) — same page components get mounted under this layout in
// App.tsx, since every one of them only ever reads `installation` off
...
export function TenantDashboardLayout() {
  const { state, installation, logout } = useTenantSession();
  ...
  if (state === "anonymous") {
    return <Navigate to="/tenant/login" replace state={{ from: location.pathname }} />;
  }
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl">
        <aside className="hidden w-60 shrink-0 flex-col border-r border-border/70 p-4 sm:flex">
          <div className="mb-6 flex items-center gap-2.5 px-1">
            ...
            <span className="font-display text-[14px] font-semibold tracking-tight text-ink">Dashboard</span>
          </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="flex items-center justify-between border-b border-border/70 px-5 py-3 sm:px-7">
            <span className="font-display text-sm font-medium text-ink-muted">Dashboard</span>
            <ThemeToggle />
          </header>
