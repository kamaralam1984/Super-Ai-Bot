...
/**
 * Modeled on CompletionStep.tsx, with two tenant-specific differences:
 * the embed snippet carries this tenant's own data-installation-id (see
 * widget.js's own doc comment — a public, non-secret identifier), and the
 * "open dashboard" button lands on /app (the tenant dashboard) instead of
 * /dashboard (the platform super-admin's).
 */
export function OnboardingCompleteStep({ businessName, websiteUrl, installationId }: { businessName: string; websiteUrl: string; installationId: string }) {
  const navigate = useNavigate();
  const [copied, setCopied] = useState(false);

  const embedSnippet = `<script src="${window.location.origin}/widget.js" data-installation-id="${installationId}"></script>`;
  ...
  return (
    ...
    >
      ...
      <h2 className="mt-5 font-display text-xl font-semibold text-ink">You're all set</h2>
      <p className="mt-1 text-sm text-ink-muted">{businessName} is now connected and ready.</p>
      ...
      <div className="mt-6 grid grid-cols-1 gap-2 text-left">
        {[
          ...
          { icon: ShieldCheck, text: "Your data is isolated from every other account on this platform" },
      ...
      <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
        <PrimaryButton onClick={() => navigate("/app")}>
