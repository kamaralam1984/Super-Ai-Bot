...
(function () {
  ...
  // Check the installation is actually live before showing anything at
  // all. Without this, a disabled/missing/unreachable installation still
  // shows a launcher bubble that just opens into a "Chat unavailable"
  // panel — technically correct (chat really is unavailable) but reads
  // to a site owner as "I deleted the chatbot and it's still there."
  // Fully absent is the only state that matches "Delete Chatbot"'s promise.
  var configUrl = origin + "/api/chat/config" + (installationId ? "?installationId=" + encodeURIComponent(installationId) : "");
  fetch(configUrl)
    .then(function (res) {
    ...
    .then(function (body) {
      if (!body || !body.success) throw new Error("config unavailable");
      init();
    })
    .catch(function () {
      // Disabled installation, not-yet-completed install, or a transient
      // network error — in every case, stay fully invisible.
    });
